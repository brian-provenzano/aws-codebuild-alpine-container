...
AMI_COUNT=$(aws ec2 describe-images --owners self --filters 'Name=tag:Purpose,Values=web' --profile hostingdemo-fulladmin --query 'Images[*].{ID:ImageId}' --output text | wc -l)
if [[$AMICOUNT -gt 1]]; then
    aws ec2 describe-images --owners self --filters 'Name=tag:Purpose,Values=web' --profile hostingdemo-fulladmin --query 'sort_by(Images, &CreationDate)[0].[ImageId]' --output text > /root/ami-delete.txt
